...
def generate_html_from_excel(file_path, output_path):
    try:
        ...
        print(df.columns)  # Debug-Zeile, um die Spaltennamen zu überprüfen
        ...
        for _, row in df.iterrows():
            code = str(row['StudentCode'])  # Korrektur: 'SchuelerCode' zu 'StudentCode'
            subject = row['Fach']
            if code not in student_data:
                student_data[code] = []

            # Mitarbeitsnoten (bis zu 34 Einträge, inklusive der neuen Kategorien)
            participation_notes = [
                {
                    "Name": row[f"MA_Name{i}"],
                    "Note": transform_participation_grade(row[f"MA_Beurteilung{i}"])
                }
                for i in range(1, 35)  # Erweitert auf 34 Einträge
            ...
            student_data[code].append({
                ...
                "GesamtTest": "" if row['GesamtTest'] == 0 else f"{row['GesamtTest']:.1f}",  # Originalformat mit einer Nachkommastelle
        ...
        <head>
            ...
            <style>
                body {{ 
                    ...
                    font-size: 14px;
                    background-color: #f0f0f0; /* Hellgrauer Hintergrund für bessere Lesbarkeit im Dark Mode */
                ...
                table {{ 
                    ...
                    background-color: #fff; /* Weißer Hintergrund für die Tabelle */
                    color: #333; /* Dunkle Schriftfarbe */
                    box-shadow: 0 0 10px rgba(0, 0, 0, 0.5); /* Leichter Schatten für bessere Sichtbarkeit */
                }}
                th, td {{ 
                ...
                /* Regel für kleinere Bildschirme */
                ...
                /* Stil für das Schülercode-Eingabefeld */
                input#schuelerCode {{
                    font-size: 16px; /* Schriftgröße für den Schülercode */
                    padding: 8px;    /* Optional: Abstand im Eingabefeld */
                }}
            </style>

            <script>
                // JavaScript-Daten: Schülercode und zugehörige Noten
                ...
                function updateSubjects() {{
                    ...
                    subjectDropdown.innerHTML = ""; // Leeren des Dropdown-Menüs
                    ...
                    if (studentData[codeInput]) {{
                        ...
                        subjectDropdown.style.display = "block"; // Dropdown anzeigen
                    }} else {{
                        subjectDropdown.style.display = "none"; // Dropdown ausblenden
                    }}
                    showStudentData(); // Daten aktualisieren
                }}
...
# Pfade zur Excel-Datei und zur HTML-Ausgabedatei
file_path = r"C:\Users\marku\OneDrive - Berufsschule Linz 3\Sonstiges\NotenListeGesamt\NotenAlleSchülerGesamt.xlsx"
output_path = r"C:\Users\marku\OneDrive - Berufsschule Linz 3\Sonstiges\BSDokumente\Freigegebene Dokumente - NotenKnoll\Notenübersicht.html"
